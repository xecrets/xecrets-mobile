...
commit (note the sibling repos `xecrets-net`, `xecrets-common`,
`xecrets-localization` and `xecrets-words` pin the same SDK version in their own
`global.json` files and should be moved together). When you update Xcode, update `.xcode-version` — but first
